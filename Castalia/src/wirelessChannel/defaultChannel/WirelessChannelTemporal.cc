...
channelTemporalModel::channelTemporalModel(const char *file, int rng)
{
	...
	while (getline(f, s))
	{
		...
		while ((ct = t.nextToken()))
		{
			...
			if (param_count > 2)
			{ // param_count is the number of header parameters that was parsed already
				...
				if (time == coherenceTime)
				{ // we discovered coherence pdf definition
					if (coherencePDF)
					{
				...
				{ // this is not coherence pdf, so the signal variability value is expected
					while (ct[0] && ct[0] != ',')
					{
						ct++;
					}
	...
	for (int i = 0; i < numOfCorrelationTimes; i++)
	{
		for (int j = 0; j < numOfSignalVariationValues; j++)
		{
			...
			{
				std::cout << "[TemporalModel] ERROR: missing PDF description for " << correlationTime[i].time << "," << (j * signalVariationResolution) + minSignalVariation << std::endl;
				exit(1);
			}
		}
	}
...
{
	if (!c[1] && ((c[0] >= 'a' && c[0] <= 'z') || (c[0] >= 'A' && c[0] <= 'Z')))
	{
		return 1;
	}
	return 0;
...
float channelTemporalModel::drawFromPDF(PDFType *pdf)
{
	...
	while (guard < 100)
	{ // Since a PDF is allowed to have recursive layers
